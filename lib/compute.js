/**
 * Compute Module
 * Classifies raw calls and builds all aggregation data.
 * Logic extracted from call-dashboard-app/server/index.js (unchanged).
 */
const { classifyCall, paraphraseQuote } = require("./classify");
const { buildAdvancedInsights } = require("./claude-analyze");

function computeAll(rawCalls) {
  console.log(`Computing: ${rawCalls.length} raw calls`);

  const classified = rawCalls
